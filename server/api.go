	...
	api_globals = sl.StringDict{
		"mochi": sls.FromStringDict(sl.String("mochi"), sl.StringDict{
			...
			"action": sls.FromStringDict(sl.String("action"), sl.StringDict{
				"dump":  sl.NewBuiltin("mochi.action.dump", api_action_dump),
				"error": sl.NewBuiltin("mochi.action.error", api_action_error),
				"file": sls.FromStringDict(sl.String("file"), sl.StringDict{
					"name":   sl.NewBuiltin("mochi.action.file.name", api_action_file_name),
					"upload": sl.NewBuiltin("mochi.action.file.upload", api_action_file_upload),
				}),
				"json":     sl.NewBuiltin("mochi.action.json", api_action_json),
				"redirect": sl.NewBuiltin("mochi.action.redirect", api_action_redirect),
				"write":    sl.NewBuiltin("mochi.action.write", api_action_write),
			}),
...
func api_action_dump(t *sl.Thread, fn *sl.Builtin, args sl.Tuple, kwargs []sl.Tuple) (sl.Value, error) {
	var vars []any
	for _, v := range args {
...
// Print an error
func api_action_error(t *sl.Thread, fn *sl.Builtin, args sl.Tuple, kwargs []sl.Tuple) (sl.Value, error) {
	if len(args) != 2 {
		return sl_error(fn, "syntax: <code: integer> <message: string>")
	}

	code, err := sl.AsInt32(args[0])
	if err != nil {
		return sl_error(fn, "invalid error code")
	}

	message, ok := sl.AsString(args[1])
	if !ok {
		return sl_error(fn, "invalid error message")
	}
...
func api_action_file_name(t *sl.Thread, fn *sl.Builtin, args sl.Tuple, kwargs []sl.Tuple) (sl.Value, error) {
	if len(args) != 1 {
	...
	if err != nil {
		return sl_error(fn, "unable to get name of file field '%s': %v", field, err)
	}

	return sl_encode(ff.Filename), nil
}

// Write the contents of an uploaded file
func api_action_file_upload(t *sl.Thread, fn *sl.Builtin, args sl.Tuple, kwargs []sl.Tuple) (sl.Value, error) {
	if len(args) != 2 {
	...
	err = a.web.SaveUploadedFile(ff, api_file(user, app, file))
	if err != nil {
...
func api_action_json(t *sl.Thread, fn *sl.Builtin, args sl.Tuple, kwargs []sl.Tuple) (sl.Value, error) {
	if len(args) != 1 {
		return sl_error(fn, "syntax: <data: dictionary>")
	}
	...
	a.json(sl_decode(args[0]))

	return sl.None, nil
}

// Redirect the action
func api_action_redirect(t *sl.Thread, fn *sl.Builtin, args sl.Tuple, kwargs []sl.Tuple) (sl.Value, error) {
	if len(args) < 1 {
...
func api_action_write(t *sl.Thread, fn *sl.Builtin, args sl.Tuple, kwargs []sl.Tuple) (sl.Value, error) {
	if len(args) < 2 || len(args) > 3 {
		return sl_error(fn, "syntax: <template path: string> <format: string> [data: dictionary]")
	}
	...
	if !ok {
		return sl_error(fn, "invalid format '%s'", format)
	}
	...
	case "json":
		if len(args) < 3 {
			return sl_error(fn, "JSON called without data")
		}
		a.json(sl_decode(args[2]))
	...
	default:
		...
		if len(args) > 2 {
			err = tmpl.Execute(a.web.Writer, sl_decode(args[2]))
		} else {
...
func api_service_call(t *sl.Thread, fn *sl.Builtin, args sl.Tuple, kwargs []sl.Tuple) (sl.Value, error) {
	...
	if depth > 1000 {
		return sl_error(fn, "reached maximum recursion depth")
	}
