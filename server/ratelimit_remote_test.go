...
func TestRemoteRateLimitPerTarget(t *testing.T) {
	...
	for i := 0; i < limit; i++ {
		if reason := remote_rate_limit(thread, "1victim"); reason != "" {
			t.Fatalf("call %d of %d refused (%q); the limit must clear real use - a profile view is five fetches", i+1, limit, reason)
		}
	}
	...
	if !strings.Contains(reason, "per target") {
		t.Errorf("refusal is %q, want the per-target message - otherwise the aggregate limit tripped first and this test proves nothing about per-target", reason)
	}
}
...
func TestRemoteRateLimitReportsTheEnforcedNumber(t *testing.T) {
	for _, c := range []struct {
		...
		refuse  func(*sl.Thread) string
	}{
		{"per target", rate_limit_remote_entity, func(th *sl.Thread) string {
			for i := 0; i < rate_limit_remote_entity.limit+1; i++ {
				if reason := remote_rate_limit(th, "1victim"); reason != "" {
					return reason
				}
			}
			return ""
		}},
		{"aggregate", rate_limit_remote, func(th *sl.Thread) string {
			for i := 0; i < rate_limit_remote.limit+1; i++ {
				if reason := remote_rate_limit(th, "1entity"+strconv.Itoa(i)); reason != "" {
					return reason
				}
			}
			return ""
		...
		t.Run(c.name, func(t *testing.T) {
			...
			if !strings.Contains(reason, want) {
				t.Errorf("refusal is %q but the enforced limit is %s; the message must name the limit actually applied", reason, want)
			}
		})
	}
}
...
func TestRemoteRateLimitIsolatesTargets(t *testing.T) {
	...
	if reason := remote_rate_limit(thread, "1flooded"); reason == "" {
		t.Fatalf("the flooded target is not limited, so this test cannot show isolation")
	}

	if reason := remote_rate_limit(thread, "1bystander"); reason != "" {
		t.Errorf("an unrelated target was refused (%q); one entity's traffic must not starve another's", reason)
	}
}
...
func TestRemoteRateLimitIsolatesApps(t *testing.T) {
	...
	if reason := remote_rate_limit(flooding, "1victim"); reason == "" {
		t.Fatalf("the flooding app is not limited, so this test cannot show isolation")
	}

	if reason := remote_rate_limit(remote_limit_thread("feeds"), "1victim"); reason != "" {
		t.Errorf("a different app was refused (%q); budgets are per app", reason)
	}
}
...
func TestRemoteRateLimitAggregate(t *testing.T) {
	...
	for i := 0; i < rate_limit_remote.limit+1; i++ {
		if reason := remote_rate_limit(thread, "1entity"+strconv.Itoa(i)); reason != "" {
			refused = reason
			break
		}
	}
	...
	if strings.Contains(refused, "per target") {
		t.Errorf("refusal is %q, want the aggregate message - each target was distinct so per-target should not have fired", refused)
	}
}
...
func TestRemoteRateLimitInternalCallsUncharged(t *testing.T) {
	...
	for i := 0; i < rate_limit_remote.limit+rate_limit_remote_entity.limit+10; i++ {
		if reason := remote_rate_limit(bare, "1victim"); reason != "" {
			t.Fatalf("an appless thread was refused at call %d (%q)", i+1, reason)
		}
	}
}
...
func TestRemoteRateLimitClearsRealUse(t *testing.T) {
	...
	for i := 0; i < views; i++ {
		for j := 0; j < fetches_per_view; j++ {
			if reason := remote_rate_limit(thread, "1person"); reason != "" {
				t.Fatalf("refused during view %d of %d (%q); %d views of one profile is ordinary use, not abuse",
...
func TestRemoteRateLimitClearsUpdateSweep(t *testing.T) {
	...
	for i := 0; i < catalogue; i++ {
		if reason := remote_rate_limit(thread, "1publisher"); reason != "" {
			t.Fatalf("the update sweep was refused at app %d of %d (%q); a cold-cache sweep of a %d-app catalogue must complete, and a refusal here aborts the whole action rather than skipping one app",
