...
func is_v2_unsupported(err error) bool {
	...
	if msg := err.Error(); len(msg) > 0 {
		if matches := stringContainsAll(msg, "does not support"); matches {
			return true
		}
	}
	return false
...
// stringContainsAll is a tiny std-lib avoider that mirrors
// strings.Contains so we don't drag strings into protocol2_sender.go
// just for one call.
func stringContainsAll(s, sub string) bool {
	if len(sub) > len(s) {
		return false
	}
